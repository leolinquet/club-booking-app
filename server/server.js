...
app.get('/tournaments/:id', async (req, res) => {
  try {
    ...
    const tournament = await db.prepare(`
      SELECT id, club_id, name, sport, draw_size, seed_count, seeds_count, points_by_round, end_date
      FROM tournaments
      WHERE id = ?
    `).get(tId);
    ...
    const normTournament = {
      ...
      draw_size: tournament.draw_size != null ? Number(tournament.draw_size) : null,
      seed_count: tournament.seed_count != null ? Number(tournament.seed_count) : null,
      seeds_count: tournament.seeds_count != null ? Number(tournament.seeds_count) : null,
      points_by_round: tournament.points_by_round || null,
...
// PATCH /tournaments/:id/settings - Update tournament settings (draw size, seeds, points)
app.patch('/tournaments/:id/settings', async (req, res) => {
  try {
    const tId = Number(req.params.id);
    const { managerId, drawSize, seeds, points } = req.body;
    ...
    const clubRow = await db.prepare('SELECT manager_id FROM clubs WHERE id=?').get(t.club_id);
    if (!clubRow || Number(clubRow.manager_id) !== Number(managerId)) {
      return res.status(403).json({ error: 'only club manager can update tournament settings' });
    }

    // Update tournament settings
    await db.prepare(`
      UPDATE tournaments 
      SET draw_size = ?, seed_count = ?, points_by_round = ?
      WHERE id = ?
    `).run(
      Number(drawSize), 
      Number(seeds), 
    ...
    console.error('update tournament settings error', e);
    res.status(500).json({ error: 'unexpected error' });
  }
});
